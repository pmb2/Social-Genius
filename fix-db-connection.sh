...
# Create a script to fix the database connection issues
echo "Fixing database connection issues..."

# 1. Update the PostgresService.ts file to use direct IP connection
cat > /home/ubuntu/Social-Genius/services/postgres-service-fixed.ts << 'EOF'
...
# Update the init-db.sql file
cat > /home/ubuntu/Social-Genius/init-db.sql << 'EOF'
-- Create vector extension (for embeddings)
CREATE EXTENSION IF NOT EXISTS vector;

-- Create users table with profile_picture and phone_number columns
CREATE TABLE IF NOT EXISTS users (
  id SERIAL PRIMARY KEY,
  email TEXT UNIQUE NOT NULL,
  password_hash TEXT NOT NULL,
...
# Create a modified docker-compose-fixed.yml file
cat > /home/ubuntu/Social-Genius/docker-compose-fixed.yml << 'EOF'
version: '3.8'
services:
  ...
  app:
    ...
    command: >
      sh -c "echo 'Waiting for database to be ready...' &&
             ...
             NEXT_PUBLIC_HOST=http://18.223.171.255:3000 npm run dev -- -H 0.0.0.0"
...
cat > /home/ubuntu/Social-Genius/fixed-start-dev.sh << 'EOF'
#!/bin/bash
...
chmod +x /home/ubuntu/Social-Genius/fixed-start-dev.sh
chmod +x /home/ubuntu/Social-Genius/fix-db-connection.sh

echo "Fix scripts created successfully!"
echo "To run the fixed version, use: ./fixed-start-dev.sh"