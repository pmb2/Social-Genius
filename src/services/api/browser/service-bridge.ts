...
import { EncryptedCredential, unwrapCredentials } from '../utilities/credentials-manager';
...
async function loadBrowserManager(): Promise<any> {
  ...
  try {
    ...
    const module = await import('../compliance/browser-manager');
...
async function loadConsolidatedAuthService(): Promise<any> {
  try {
    // Dynamic import to avoid circular dependencies
    const { ComplianceAuthService } = await import('../compliance/consolidated-auth-service');
