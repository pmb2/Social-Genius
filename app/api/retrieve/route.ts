...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      ...
      // Apply similarity threshold filtering manually with optional document filtering
      // For this, we need to use the vectorStore directly to get scores
      
      // Define search filter for specific document IDs if needed
      ...
      // Use a standard number parameter for k and pass filter separately
      const results = await vectorStore.similaritySearchWithScore(query, 10, filter);
