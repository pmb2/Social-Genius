...
echo -e "${YELLOW}Starting Social Genius in production mode...${NC}"
...
if [ ! -z "$EXISTING_CONTAINERS" ]; then
  echo -e "${YELLOW}Found existing Social Genius containers, automatically stopping them...${NC}"
...
if [ $? -eq 0 ]; then
  ...
  echo -e "${GREEN}Application is now running at: http://localhost:3000${NC}"
  ...
  if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    echo -e "${GREEN}To view logs later, run: docker-compose logs -f app${NC}"
  fi
else
